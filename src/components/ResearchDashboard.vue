<template>
  <div
    class="min-h-full bg-[radial-gradient(circle_at_top_left,_rgba(59,130,246,0.14),_transparent_28%),radial-gradient(circle_at_top_right,_rgba(251,191,36,0.18),_transparent_30%),linear-gradient(180deg,_#f8fafc_0%,_#eef2ff_100%)]"
  >
    <div class="mx-auto max-w-screen-xl px-6 py-6">
      <section
        class="rounded-3xl border border-slate-200 bg-white/85 p-6 shadow-[0_20px_60px_rgba(15,23,42,0.08)] backdrop-blur"
      >
        <div class="flex flex-wrap items-start justify-between gap-6">
          <div class="max-w-3xl">
            <div
              class="text-xs font-semibold uppercase tracking-[0.28em] text-sky-700"
            >
              Multi-Flop Research
            </div>
          ...
          <div
            class="grid min-w-[17rem] gap-3 rounded-2xl border border-slate-200 bg-slate-950 px-5 py-4 text-slate-50 shadow-xl"
          >
      ...
      <section class="mt-6 grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div class="mt-5 grid gap-4 lg:grid-cols-2">
            ...
            >
              ...
              <div
                class="mt-4 grid grid-cols-3 gap-3 text-left text-sm text-slate-700"
              >
                <div>
                  <div
        ...
        <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <p v-if="!supportsSharedThreads" class="mt-2 text-sm text-amber-700">
            Multi-thread WASM indisponible dans ce contexte navigateur. La
            recherche utilise 1 thread.
          </p>
          <p
            v-if="workerRuntimeText"
            class="mt-2 text-sm"
            :class="workerRuntimeClass"
          >
            {{ workerRuntimeText }}
          </p>
          <p
            v-if="researchInitError"
          ...
          <div
            v-if="invalidCustomBoards.length > 0"
            ...
            <div class="flex items-center justify-between gap-4">
              <div>
                ...
                <div class="mt-1 text-xs text-slate-500">
                  Liste exacte qui sera lancée par la batch: sélection
                  représentative du bas + custom flops valides.
                </div>
              </div>
          ...
          <label
            class="mt-5 flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-700"
          >
            <input
            ...
            <span class="font-normal text-slate-500">
              Ignore le cache persistant de cette situation et relance tous les
              flops.
            </span>
          </label>
          ...
          <div
            class="mt-5 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700"
          >
          ...
          >
            ...
            <div class="min-w-0 flex-1">
              <div class="flex items-center justify-between gap-2">
                ...
                <div
                  class="rounded-full bg-slate-900 px-2.5 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-white"
                >
                  w {{ flop.weight }}
                </div>
              </div>
              <div
                class="mt-1 text-xs font-semibold uppercase tracking-[0.18em] text-sky-700"
              >
                {{ flop.category }}
              </div>
              ...
              <div
                v-if="flop.flopBetOverride"
                class="mt-3 text-xs font-semibold uppercase tracking-[0.18em] text-amber-700"
              >
      ...
      >
        <div class="flex items-center gap-3 text-slate-700">
          ...
          <span class="text-sm font-semibold"
            >Loading persisted research results...</span
          >
        </div>
      ...
      <section
        ...
        class="mt-6 grid gap-6 xl:grid-cols-[0.9fr_1.1fr]"
      >
        <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div class="mt-5 grid gap-5 lg:grid-cols-2">
                ...
                >
                  ...
                  <div class="mt-3 h-3 rounded-full bg-slate-200">
                    <div
                      ...
                      :style="{
                        width: `${Math.max(2, item.frequency * 100)}%`,
                      }"
                    ></div>
                  </div>
                </div>
            ...
            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              ...
              <div class="mt-1 text-xs text-slate-500">
                Premier spot IP sur le flop, en pratique après check OOP.
              </div>
              <div class="mt-4 space-y-4">
                <div
      ...
      >
        ...
        <div class="mt-5 space-y-4">
          ...
          >
            <div class="flex flex-wrap items-start justify-between gap-4">
              <div>
                <div class="flex items-center gap-3">
                  ...
                  <div
                    class="rounded-full bg-slate-900 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-white"
                  >
                ...
                <div class="rounded-2xl bg-slate-50 px-4 py-3">
                  ...
                  <div class="mt-1 text-lg font-bold text-slate-900">
                    {{
                      result.error
                        ? "-"
                        : formatAdaptive(result.outcome!.exploitability)
                    }}
                  </div>
                </div>
                <div class="rounded-2xl bg-slate-50 px-4 py-3">
                  ...
                  <div class="mt-1 text-lg font-bold text-slate-900">
                    {{
                      result.error
                        ? "-"
                        : formatMs(result.outcome!.elapsedTimeMs)
                    }}
                  </div>
                </div>
                <div class="rounded-2xl bg-slate-50 px-4 py-3">
                  ...
                  <div class="mt-1 text-lg font-bold text-slate-900">
                    {{
                      result.error
                        ? "-"
                        : formatPercent(result.outcome!.rootSummary.equity[0])
                    }}
                  </div>
                </div>
                <div class="rounded-2xl bg-slate-50 px-4 py-3">
                  ...
                  <div class="mt-1 text-lg font-bold text-slate-900">
                    {{
                      result.error
                        ? "-"
                        : formatAdaptive(result.outcome!.rootSummary.ev[0])
                    }}
                  </div>
                </div>
              </div>
            ...
            <div
              v-if="result.error"
              class="mt-4 rounded-2xl bg-red-100 px-4 py-3 text-sm font-semibold text-red-700"
            >
              {{ result.error }}
            </div>
            ...
            <div v-else class="mt-5 grid gap-4 lg:grid-cols-2">
                ...
                <div class="mt-3 space-y-3">
                  ...
                  >
                    <div class="flex items-start justify-between gap-3">
                      ...
                      <div
                        class="text-right text-2xl font-black text-slate-900"
                      >
              ...
              <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                ...
                <div class="mt-1 text-xs text-slate-500">
                  {{ getFlopPlayerContext(result, "ip") }}
                </div>
                <div class="mt-3 space-y-3">
                  <div
            ...
            >
              <button
                class="button-base button-red"
                :disabled="isRunning || isDeletingCache"
                @click="deleteFlopCache(result)"
              >
                Delete Flop Cache
              </button>
              <button
                class="button-base button-blue"
                @click="openResultPage(result)"
              >
                Open Result Page
              </button>
            </div>
...
const researchPresetCacheKey = (presetId: ResearchPresetId) =>
  isSingleRaisedPotResearchPreset(presetId)
    ? `${presetId}:research-16bit`
    : presetId;
...
export default defineComponent({
  setup() {
    ...
    const activeFlops = computed<ActiveFlopStudyItem[]>(() => {
      const base = flops.filter((flop) =>
        selectedFlopIds.value.includes(flop.id)
      );
      const custom = customFlops.value.filter(
        (flop) => flop.parsedBoard.length === 3
      );
      return [...base, ...custom];
    });

    const selectedRepresentativeFlopCount = computed(
      () => selectedFlopIds.value.length
    );

    const researchInitError = computed(
      () => selectedPresetSnapshotState.value.error
    );
    ...
    const treeKey = computed(() => {
      ...
      return computeTreeKeyFromSnapshot(
        snapshot,
        researchPresetCacheKey(selectedPresetId.value)
      );
    });
    ...
    const situationKey = computed(() => {
      ...
      return computeSituationKeyFromSnapshot(
        snapshot,
        researchPresetCacheKey(selectedPresetId.value),
        {
    ...
    const activeFlopDescriptors = computed(() => {
      return activeFlops.value.map((flop) => {
        const treeOverrides: FlopTreeOverrides | undefined =
          flop.flopBetOverride
            ? {
    ...
    const effectiveFlopListText = computed(() => {
      ...
      return activeFlops.value
        .map((flop, index) => {
          ...
          return `${String(index + 1).padStart(2, "0")}. ${flop.boardText} | ${
            flop.label
          } | ${source}`;
        })
        .join("\n");
    ...
    const aggregatePlayerActions = (
      ...
      for (const result of results.value) {
        ...
        for (const action of getResultFlopPlayerSummary(result, player)
          .actions) {
      ...
      return Array.from(map.entries())
        .map(([label, value]) => ({
          ...
          frequency:
            value.totalWeight > 0
              ? value.weightedFrequency / value.totalWeight
              : 0,
          ev:
            value.totalWeight > 0
              ? value.weightedEv / value.totalWeight
              : Number.NaN,
          count: value.count,
        }))
    ...
    const progressTitle = computed(() =>
      isRunning.value
        ? `Batch running${
            activeRunPresetLabel.value ? `: ${activeRunPresetLabel.value}` : ""
          }`
        : "Ready"
    );
    const progressText = computed(() => statusText.value);
    const workerRuntimeClass = computed(() =>
      workerRuntimeText.value.includes("fallback")
        ? "text-amber-700"
        : "text-slate-600"
    );
    ...
    const loadSelectedPreset = () => {
      if (isRunning.value) {
        statusText.value =
          "Cannot load a preset into Solver while a batch is running.";
        return;
      }
    ...
    const toggleFlop = (flopId: string) => {
      if (selectedFlopIds.value.includes(flopId)) {
        selectedFlopIds.value = selectedFlopIds.value.filter(
          (item) => item !== flopId
        );
      } else {
    ...
    const deletePresetCache = async () => {
      if (!treeKey.value || isRunning.value || isDeletingCache.value) {
        return;
      }

      isDeletingCache.value = true;
      try {
        const deleted = await deleteResearchRuns(
          situationKey.value,
          treeKey.value
        );
        if (!deleted) {
    ...
    const stopResearch = () => {
      ...
      statusText.value =
        "Stopping current batch after the current solver step...";
    };
    ...
    const openResultPage = (result: BatchResult) => {
      window.open(
        buildResearchResultUrl(toResearchResultSelection(result)),
        "_blank",
        "noopener"
      );
    };
    ...
    const deleteFlopCache = async (result: BatchResult) => {
      ...
      isDeletingCache.value = true;
      try {
        const currentTreeKey = computeTreeKey(
          config,
          researchPresetCacheKey(selectedPresetId.value)
        );
        const deleted = await deleteResearchRun(
          result.situationKey,
          result.flopKey,
          currentTreeKey
        );
    ...
    const syncResultsFromCache = async () => {
      ...
      if (!situationKey.value) {
        ...
        if (researchInitError.value) {
          ...
          statusText.value =
            "No cached results are available for the current preset selection yet.";
        }
      ...
      try {
        ...
        results.value = activeFlopDescriptors.value.flatMap((descriptor) => {
          ...
          return record
            ? [toBatchResult(situationKey.value, descriptor, record)]
            : [];
        });
      } catch (error) {
        results.value = [];
    ...
    const getFlopPlayerActions = (result: BatchResult, player: "oop" | "ip") =>
      getResultFlopPlayerSummary(result, player).actions;

    const getFlopPlayerContext = (result: BatchResult, player: "oop" | "ip") =>
      getResultFlopPlayerSummary(result, player).contextLabel ||
      "First flop decision";
    ...
    const runResearch = async () => {
      ...
      for (const [index, flop] of runFlops.entries()) {
        ...
        const board: number[] =
          "parsedBoard" in flop
            ? flop.parsedBoard
            : parseStudyBoard(flop.boardText);
        const treeOverrides: FlopTreeOverrides | undefined =
          flop.flopBetOverride
            ? {
                oopFlopBet: flop.flopBetOverride,
            ...
            : undefined;
        const cachePresetId = researchPresetCacheKey(runPreset.id);
        const treeKey = computeTreeKey(config, cachePresetId);
        ...
        if (cachedMap.has(flopKey)) {
          ...
          if (!cachedValue.snapshot) {
            await putResearchRun({
              ...
              presetLabel:
                cachedMap.get(flopKey)?.presetLabel || runPreset.label,
          ...
          statusText.value = `CACHE ${index + 1}/${runFlops.length} ${
            flop.label
          }`;
          continue;
        }
        ...
        try {
          const solveResult: BatchSolveResult = await runBatchSolve(snapshot, {
            ...
            onProgress: (progress) => {
              ...
              statusText.value = `${progress.stage.toUpperCase()} ${
                progress.flopIndex
              }/${progress.flopCount} ${progress.flopLabel} | iteration ${
                progress.currentIteration
              }/${progress.maxIterations} | exploitability ${formatAdaptive(
          ...
          outcome.rootSummary.actions = normalizeActions(
            outcome.rootSummary.actions
          );
          const previousValue = existingRecord?.value as
            | PersistedBatchValue
            | undefined;
          ...
          const serializedGame =
            toPersistedSerializedGame(solveResult.serializedGame) ||
            previousSerializedGame;
          refreshWorkerRuntimeText();
          ...
          const persistedRecord = persisted
            ...
            : undefined;
          const persistedValue = persistedRecord?.value as
            | PersistedBatchValue
            | undefined;
          ...
          if (!persisted) {
            ...
            solveResult.serializedGame &&
            !persistedHasSerializedGame
          ) {
            statusText.value = `Solved ${flop.label}, but the native result tree was not persisted.`;
          } else if (
            solveResult.serializationError &&
            !previousSerializedGame
          ) {
